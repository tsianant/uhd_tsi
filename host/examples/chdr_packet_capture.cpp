                               ...
                               double sample_rate = 20e6)
{
    try {
        ...
        if (siggen0) {
            ...
            std::cout << "SigGen#0 configured: " << siggen0_freq/1e6 << " MHz, amplitude " << siggen0_ampl << std::endl;
        }
        ...
        if (siggen1) {
            ...
            std::cout << "SigGen#1 configured: " << siggen1_freq/1e6 << " MHz, amplitude " << siggen1_ampl << std::endl;
        }
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    // Configure radio (even if using signal generators, for timing reference)
    std::cout << "Configuring radio " << radio_id << ", channel " << radio_chan << std::endl;
    if (signal_path == PATH_RADIO) {
    ...
    // Configure signal generators if needed
    if (signal_path == PATH_SIGGEN_0 || signal_path == PATH_SIGGEN_1 || signal_path == PATH_ADDSUB) {
        configure_signal_generators(graph, siggen0_freq, siggen0_ampl, siggen1_freq, siggen1_ampl, rate);
    }
    
    // Configure switchboards for the selected path
    configure_switchboards(graph, signal_path, custom_config);
